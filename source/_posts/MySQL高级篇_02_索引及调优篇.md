...
* 以其他列作为搜索条件
  ...
  因为在数据页中并没有对非主键列简历所谓的页目录，所以我们无法通过二分法快速定位相应的槽。这种情况下只能从 `最小记录` 开始 `依次遍历单链表中的每条记录`， 然后对比每条记录是不是符合搜索条件。很显然，这种查找的效率是非常低的。这就是我们常说的效率很低的**全表扫描**；

> Mysql的页与槽 https://www.cnblogs.com/detectiveHLH/p/14907405.html
>
> 为什么说数据记录是一条“逻辑上的单链表”？
>
> 如果Mysql的数据记录是严格意义上的物理磁盘层面的顺序排列，效率会很低。试想，中间插入一条数据，需要迁移很多数据，类似Java中数组的插入。
>
> 实际实现：通过数据页之间的上下指针，实现一种单链表的效果。在插入或删除数据时，通过页分裂，页合并配合，避免了迁移过多的数据。
...
![image-20240617183004815](MySQL高级篇_02_索引及调优篇/image-20240617183004815.png)
...
> Mysql的B+索引有点类似Redis的跳表结构，聚簇索引的用主键构建跳表，二级索引用索引列构建跳表。
