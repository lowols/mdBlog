...
* 将后端服务器Servlet拆分成三层，分别是`web`、`service`和`dao`
  * web层【即表现层】主要由servlet来处理，负责页面请求和数据的收集以及响应结果给前端
  * service层主要负责业务逻辑的处理
  * dao层主要负责数据的增删改查操作
...
* 因为是前后端分离，所以后端不需要返回view视图，将其去除
* 前端如果通过请求后台服务器进行交互，后台就需要将返回的数据转换成json格式进行返回
* SpringMVC==主要==负责的就是
  * 如何接收请求和数据，解析参数
  * 如何将请求和数据转发给业务层
  * 如何将响应数据转换成json发回到前端

相对于使用原生的servlet，使用SpringMVC可以更方便快捷的进行网站开发。

介绍了这么多，对SpringMVC进行一个定义

* SpringMVC是一种基于Java实现MVC模型的轻量级Web层框架，是一个表现层的框架。
...
> 前端控制器
>
> 通常使用SpringMVC时，底层默认使用一个“超级Servlet”接管所有的请求，这个Servlet叫做前端控制器或中央控制器。
>
> 前端控制器调度各种组件【映射器、处理器、适配器...】，完成请求的处理。
>
> 初始化过程就是为前端控制器运行做准备工作，ServletContainersInitConfig的作用：
>
> - 指定前端控制器的映射路径
> - 为前端控制器创建一个专属的SpringIOC容器，加载用户自定义的组件【比如Controller类的一个方法，会被解析成处理器组件，放进容器里】

ServletContainersInitConfig继承的抽象父类【AbstractDispatcherServletInitializer】会被服务器自动加载执行，父类会调用子类重写的这些扩展方法。

抽象父类主要启动过程：1，创建组件容器；2，初始化前端控制器。
...
1. 父类执行子类的createServletApplicationContext，创建一个SpringIOC容器，用来存放前端控制器依赖的一些组件。
   ...
   * 此时就建立了 `/save` 和 save方法的对应关系
   ...
   > 该容器注册配置类SpringMvcConfig，，加载用户自定义的组件【比如Controller类的一个方法，会被解析成处理器组件，放进容器里】
   >
   > 【WebApplicationContext的根接口也是BeanFactory】。
...
3. 注册前端控制器，触发init方法【抽象父类的方法】，获取上一步容器加载的的组件，完成前端控制器的初始化。

用户发起请求，前端控制器就会调度执行这些组件，完成请求的处理。

> 小贴士
>
> 模板方法模式
>
> SpringMVC框架源码中，大量应用了模板方法模式，比如ServletContainersInitConfig和它的抽象父类，前端控制器和它的抽象父类。
>
> 他们的抽象父类定义一些扩展方法给子类重写，框架启动时自动执行父类代码和子类的自定义代码。
...
> 小贴士
>
> 和Mybaits类似，为了更方便与Spring整合，SpringMVC提供一个ServletContainersInitConfig给用户。
>
> 在这个类里，我们创建了一个基于SpringIOC的容器，来放置Controller的bean对象。
>
> 同时，执行哪些请求由SpringMVC接管【DispatchServelet】。
